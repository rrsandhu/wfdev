...
import { commercialProperties } from "@/lib/data/commercial";

export function generateStaticParams() {
  return commercialProperties.map((p) => ({ slug: p.id }));
}
...
export async function generateMetadata({
  ...
  const property = commercialProperties.find((p) => p.id === slug);
  if (!property) return {};
  return {
    title: `${property.name} | Commercial | West Fraser Developments`,
    ...
    <>
            ...
            <div className="border border-gray-100 rounded-xl p-8 sticky top-28">
              ...
              <dl className="space-y-4 mb-8">
                {[
                  ...
                  { label: "Size", value: property.size },
                  { label: "Year", value: property.year },
                  { label: "Address", value: property.address ?? property.location },
                  ...(property.leaseContact ? [{ label: "Leasing", value: property.leaseContact }] : []),
                ].map((d) => (
      ...
      <section className="py-20 bg-gray-900 text-center px-6">
        ...
        <p className="text-gray-400 text-[15px] max-w-md mx-auto mb-10">
          Our commercial leasing team is ready to discuss available spaces, lease terms, and how we can support your business.
        </p>
